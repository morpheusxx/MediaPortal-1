...
namespace Mediaportal.TV.Server.SetupTV.Sections
{
  public partial class TvRecording : SectionSettings
  {
    ...
    public override void SaveSettings()
    {
      ...
      s.SaveValue(Consts.SETTINGS_KEY_PRE_RECORD_INTERVAL, (int)numericUpDownPreRec.Value);
      s.SaveValue(Consts.SETTINGS_KEY_POST_RECORD_INTERVAL, (int)numericUpDownPostRec.Value);
      ...
      try
      {
        ...
        try
        {
          ...
          if (lookupChannel != null)
          {
            ...
            lookupChannel.ChannelId.ToString();
          }
        }
        catch (Exception) { }
    ...
    private void btnImport_Click(object sender, EventArgs e)
    {
      foreach (TreeNode tagRec in tvTagRecs.Nodes)
      {
        if (tagRec.Checked) // only import the recordings which the user has selected
        {
          ...
          if (currentTagRec != null && currentTagRec.ChannelId != -1)
          {
        ...
        if (currentDbRec != null)
        {
          if (!File.Exists(currentDbRec.FileName))
          {
                              ...
                              MessageBoxDefaultButton.Button2) == DialogResult.Yes)
            {
              try
              {
                ServiceAgents.Instance.RecordingServiceAgent.DeleteRecording(currentDbRec.RecordingId);
              }
              catch (Exception ex)
              {
